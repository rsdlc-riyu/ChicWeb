...
socket.on("classification_result", (data) => {
    ...
    socket.on("max_duration", (data) => {
        ...
        // Function to fetch and display history logs
        function fetchAndDisplayHistoryLogs() {
            fetch('/get_history_logs')
                .then((response) => response.json())
                .then((data) => {
                    const historyLogsContainer = document.getElementById('historyLogsContainer');
                    historyLogsContainer.innerHTML = ''; // Clear the container
    
                    data.forEach((log) => {
                        const logCard = createHistoryLogCard(log);
        ...
        function generatePDF() {
            ...
            // Get the history logs data from the backend
            fetch('/get_history_logs')
              .then((response) => response.json())
              .then((data) => {
                // Add content to the PDF
                doc.text('History Logs', 10, 10);
                doc.setFontSize(12);
                let y = 20;
          
                data.forEach((log) => {
                  doc.text(`Date: ${log.date} | Time: ${log.time}`, 10, y);
                  y += 10;
                  doc.text(`Distress Type: ${log.distress_type}`, 10, y);
                  y += 10;
                  doc.text(`Average Frequency: ${log.average_frequency.toFixed(2)} Hz`, 10, y);
                  y += 10;
                  doc.text(`Average Magnitude: ${log.average_magnitude.toFixed(2)}`, 10, y);
                  y += 10;
                  doc.text(`Max Duration: ${log.max_duration.toFixed(2)} seconds`, 10, y);
                  y += 15;
                });
          
                // Save the PDF
                doc.save('history_logs.pdf');
              })
              .catch((error) => {
                console.error('Error fetching history logs:', error);
        ...
        // Function to create a history log card
        function createHistoryLogCard(log) {
            const logCard = document.createElement('div');
            logCard.classList.add('elevated-container', 'history-log-card');
    
            const logDate = document.createElement('p');
            logDate.textContent = `Date: ${log.date} | Time: ${log.time}`;
            logCard.appendChild(logDate);
    
            const logDistressType = document.createElement('p');
            logDistressType.textContent = `Distress Type: ${log.distress_type}`;
            logCard.appendChild(logDistressType);
    
            const logAverageFrequency = document.createElement('p');
            logAverageFrequency.textContent = `Average Frequency: ${log.average_frequency.toFixed(2)} Hz`;
            logCard.appendChild(logAverageFrequency);
    
            const logAverageMagnitude = document.createElement('p');
            logAverageMagnitude.textContent = `Average Magnitude: ${log.average_magnitude.toFixed(2)}`;
            logCard.appendChild(logAverageMagnitude);
    
            const logMaxDuration = document.createElement('p');
            logMaxDuration.textContent = `Max Duration: ${log.max_duration.toFixed(2)} seconds`;
            logCard.appendChild(logMaxDuration);
    
            return logCard;
        }
    
        // Call the function to fetch and display history logs on page load
        fetchAndDisplayHistoryLogs();
